<script>
	...
	function getTableOfContents() {
		const postTableOfContentsEl = document.querySelector('#table-of-contents + ul');
	...
	<aside>
		<section>
			{#if !showSidebar}
				...
				>
					<Icons.chevronDoubleLeft width={24} height={24} aria-hidden={true} />
				</button>
			{/if}
			...
			{#if showSidebar}
				<div class="table-of-contents" transition:fly={{ x: '100%', duration: 300 }}>
					<button on:click={toggleSidebar} aria-label="Hide table of contents">
						<Icons.chevronDoubleRight width={24} height={24} aria-hidden={true} />
					</button>
